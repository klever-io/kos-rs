...
impl std::fmt::Display for LdError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            LdError::Generic => write!(f, "Generic"),
            LdError::DerivationError => write!(f, "DerivationError"),
            LdError::InvalidIndex => write!(f, "InvalidIndex"),
        }
    }
}
...
fn derive_from_mnemonic(
    ...
    if index >= 0x80000000 {
        return Err(LdError::InvalidIndex);
    }
...
pub fn ecies_encrypt(
    ...
    ecies::encrypt(&rc_pub, &msg).map_err(|_| LdError::Generic)
}
...
pub fn ecies_decrypt(
    ...
    ecies::decrypt(&rc_prv, &msg).map_err(|_| LdError::Generic)
}
