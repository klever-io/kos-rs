...
function debugWallet(tag, w) {
  let data = JSON.stringify({
    chain: w.getChain(),
    baseChain: w.getBaseChain().getName(),
...
function waitForKOS() {
  ...
  function load(resolve, reject) {
    if (typeof window.kos !== "undefined") resolve(window.kos);
    else setTimeout(load.bind(this, resolve, reject), 250);
  }
}
...
window.onload = function () {
  ...
  document
    ...
    .addEventListener("change", loadMnemonicFromFile, false);
...
async function exportMnemonic(name, mnemonic) {
  ...
  try {
    const encrypted = kos.encrypt(
      kos.CipherAlgo.GMC,
      kos.toBytes(mnemonic),
      "12345678"
    );
